 ...
 * NavigationCancel
 * 本事件会在导航被取消之后触发。
 * 这可能是因为在导航期间某个路由守卫返回了 false。
 *
 * NavigationError
 * 这个事件会在导航由于意料之外的错误而失败时触发。
 *
 * 当打开了 enableTracing 选项时，这些事件也同时会记录到控制台中。
 * 由于这些事件是以 Observable 的形式提供的，
 * 所以你可以对自己感兴趣的事件进行 filter()，并 subscribe() 它们，以便根据导航过程中的事件顺序做出决策。
 *
 * 路由器中的关键词汇及其含义：
 *
 * Router（路由器）
 ...
 *
 * RouterLink（路由链接）
 * 这个指令把可点击的 HTML 元素绑定到某个路由。
 * 点击带有 routerLink 指令（绑定到字符串或链接参数数组）的元素时就会触发一次导航。
 *
 ...
 * 为每个路由组件提供提供的一个服务，
 * 它包含特定于路由的信息，比如路由参数、静态数据、解析数据、全局查询参数和全局碎片（fragment）。
 *
 * RouterState（路由器状态）
